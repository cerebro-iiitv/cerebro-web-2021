...
class Timeline extends React.Component {
  ...
  render() {
    const timeline = [...this.state.events].sort((eventA, eventB) => {
      ...
      let dateA = Date(parseInt(yearA), parseInt(monthA), parseInt(dayA), hourA, parseInt(minA), 0, 0)
      let dateB = Date(parseInt(yearB), parseInt(monthB), parseInt(dayB), hourB, parseInt(minB), 0, 0)
      console.log(dateA)
      console.log(dateB)
      if (dateA>dateB)
        return 1
